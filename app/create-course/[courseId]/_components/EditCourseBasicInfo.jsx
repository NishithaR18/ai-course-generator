...
function EditCourseBasicInfo({course,refreshData}) {
    ...
    const onUpdateHandler=async()=>{
        ...
        const result=await db.update(CourseList).set({
            courseOutput:course?.courseOutput}).where(eq(CourseList?.id,course?.id))
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <div>
                ...
                <Textarea className="h-40" defaultValue={course?.courseOutput?.course?.description} onChange={(event)=>setDescription(event?.target.value)}/>
            </div>
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
